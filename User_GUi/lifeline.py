...
# OpenAI setup
client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))  # set in env
DB_FILE = "lifeline.db"
...
def ai_triage(symptoms: str) -> str:
    """
    Try AI triage first (OpenAI). If fails, fallback to simple rule-based logic.
    """
    prompt = f"""
...
def triage():
    ...
    return render_template("triage.html", triage=triage_result, symptoms=symptoms_text, notes=notes)
